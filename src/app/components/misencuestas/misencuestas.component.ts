...
export class misEncuestasComponent implements OnInit {
    ...
    */
    ...
    responderEncuesta(idEncuesta, i) {
        ...
        const ciphertext = CryptoJS.DES.encrypt(idEncuesta.toString(), 'Nurvey123');
    ...
    vistaPrevia(encuesta) {
        ...
        for (let i = 0, len = x.length; i < len; i++) { x[i].remove(); }
    }
    ...
    copyClipboard() {
        /* Get the text field */
        const copyText = document.getElementById('modalSurveySubtitle');
